...
class MiniMaxConfig:
    ...
    def from_env(
        ...
        configured_music_dir = source.get("STORY_MUSIC_ASSET_DIR")
        return cls(
            ...
            music_model=source.get("MINIMAX_MUSIC_MODEL", "music-2.6"),
            ...
            music_generation_enabled=_truthy(
                source.get("STORY_MUSIC_AI_GENERATION_ENABLED", "true")
            ),
            story_auto_read_default_enabled=_truthy(
                source.get("STORY_TTS_AUTO_READ_DEFAULT_ENABLED", "false")
            ),
            ...
            music_asset_dir=music_asset_dir
            or (Path(configured_music_dir) if configured_music_dir else PROJECT_ROOT / "data" / "music_assets"),
        )
