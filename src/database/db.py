...
class GameDatabase:
    ...
    def get_game(self, game_id: int, user_id: Optional[int] = None) -> Optional[Game]:
        """Get game record."""
        return self._game_repo.get_game(game_id, user_id)  # type: ignore[no-any-return]

    def list_games(self, limit: int = 50, user_id: Optional[int] = None) -> List[Game]:
        """List recent games."""
        return self._game_repo.list_games(limit, user_id)  # type: ignore[no-any-return]

    def list_saved_games(self, user_id: int, limit: int = 50) -> List[Dict[str, Any]]:
        """获取用户的已保存游戏列表（包含详细信息）。"""
        return self._game_repo.list_saved_games(user_id, limit)  # type: ignore[no-any-return]

    def delete_saved_game(self, game_id: int, user_id: int) -> bool:
        """删除已保存的游戏（验证用户权限）。"""
        return self._game_repo.delete_saved_game(game_id, user_id)  # type: ignore[no-any-return]
    ...
    def save_ending(
        ...
        """Save game ending."""
        return self._game_repo.save_ending(game_id, final_state, ending_type, summary, achievements)  # type: ignore[no-any-return]

    # ==================== State Read/Write ====================

    def save_state(self, game_id: int, player_state: PlayerState) -> None:
        """Save a game state snapshot."""
        return self._state_repo.save_state(game_id, player_state)  # type: ignore[no-any-return]

    def load_game_state(self, game_id: int) -> Optional[Dict[str, Any]]:
        """Load the latest game state."""
        return self._state_repo.load_game_state(game_id)  # type: ignore[no-any-return]
    ...
    def save_decision(
        ...
        return self._decision_repo.save_decision(  # type: ignore[no-any-return]
    ...
    def get_decision_history(self, game_id: int) -> List[Decision]:
        ...
        return self._decision_repo.get_decision_history(game_id)  # type: ignore[no-any-return]

    def get_story_history(self, game_id: int, limit: int = 50) -> List[Dict[str, Any]]:
        """获取历史故事文本，用于一致性验证和关键行为核查。"""
        return self._decision_repo.get_story_history(game_id, limit)  # type: ignore[no-any-return]

    def search_story_history(
        self, game_id: int, keywords: List[str], max_results: int = 10
    ) -> List[Dict[str, Any]]:
        """搜索历史故事中包含关键词的段落。"""
        return self._decision_repo.search_story_history(game_id, keywords, max_results)  # type: ignore[no-any-return]
    ...
    def save_character_preset(
        ...
        return self._preset_repo.save_character_preset(  # type: ignore[no-any-return]
    ...
    def load_character_preset(
        ...
        return self._preset_repo.load_character_preset(preset_id, user_id)  # type: ignore[no-any-return]
    ...
    def set_active_game(self, user_id: int, game_id: int) -> bool:
        """设置用户当前活跃的游戏ID。"""
        return self._session_repo.set_active_game(user_id, game_id)  # type: ignore[no-any-return]

    def get_active_game(self, user_id: int) -> Optional[int]:
        """获取用户当前活跃的游戏ID。"""
        return self._session_repo.get_active_game(user_id)  # type: ignore[no-any-return]

    def clear_active_game(self, user_id: int) -> bool:
        """清除用户的活跃游戏ID。"""
        return self._session_repo.clear_active_game(user_id)  # type: ignore[no-any-return]
    ...
    def create_save_point(
        ...
        """创建存档点（手动存档）。"""
        return self._save_point_repo.create_save_point(game_id, user_id, player_state, save_name)  # type: ignore[no-any-return]

    def list_save_points(self, game_id: int, user_id: int) -> List[Dict[str, Any]]:
        """列出游戏的所有存档点。"""
        return self._save_point_repo.list_save_points(game_id, user_id)  # type: ignore[no-any-return]

    def load_save_point(self, state_id: int, user_id: int) -> Optional[Dict[str, Any]]:
        """加载特定存档点（时间回溯）。"""
        return self._save_point_repo.load_save_point(state_id, user_id)  # type: ignore[no-any-return]

    def delete_save_point(self, state_id: int, user_id: int) -> bool:
        """删除存档点。"""
        return self._save_point_repo.delete_save_point(state_id, user_id)  # type: ignore[no-any-return]

    def get_all_states_for_game(
        self, game_id: int, user_id: int, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """获取游戏的所有状态快照（用于时间线展示）。"""
        return self._save_point_repo.get_all_states_for_game(game_id, user_id, limit)  # type: ignore[no-any-return]
