...
class NPCAttributeStabilityValidator:
    ...
    def check_attribute_consistency(
        self, npc_name: str, descriptions: list, profile
    ) -> list:
        """对比描写与存储属性。"""
        issues = []
    ...
    def _check_behavioral_boundaries(
        self, text: str, npc_name: str, profile
    ) -> list:
        """检查NPC是否突破行为边界。"""
        issues = []
    ...
    def _check_identity_contradiction(
        ...
        for match in re.finditer(re.escape(npc_name), text):
            ...
            identity_keywords = ["公主", "圣女", "王子", "皇后", "国王", "将军", "大臣",
                                 "铁匠", "商人", "书生", "侠客", "巫师", "圣者", "祭司"]
                ...
                if (pos_trait in stored_personality and neg_trait in local_context) or \
                   (neg_trait in stored_personality and pos_trait in local_context):
                    issues.append({
                        ...
                        "described_trait": neg_trait if pos_trait in stored_personality else pos_trait,
                        "context": local_context[:60],
