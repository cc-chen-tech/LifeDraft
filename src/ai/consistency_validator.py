    ...
    def _parse_validation_response(self, response: str, language: str) -> ValidationResult:
        ...
        try:
            ...
            for raw in raw_issues:
                ...
                if reasoning:
                    fix_suggestion = f"{fix_suggestion}（判断理由：{reasoning}）" if language == "zh" else f"{fix_suggestion} (Reasoning: {reasoning})"
            ...
            if issues:
                ...
                logger.info(f"一致性校验结果: {'通过' if passed else '不通过(AI判断)'} "
