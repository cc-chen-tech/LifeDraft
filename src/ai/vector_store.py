...
class VectorStore:
    ...
    def add_story(
        ...
        try:
            ...
            logger.info(f"[VectorStore] Added story: id={story_id}, len={len(content)}, metadata={metadata}")
            return True
        except Exception as e:
    ...
    def search(
        ...
        try:
            ...
            if search_results:
                ...
                logger.info(f"[VectorStore] Found {len(search_results)} results, scores: {[f'{s:.3f}' for s in scores]}")
            else:
    ...
    def get_relevant_context(
        ...
        context_str = "\n".join(context_parts)
        logger.info(f"[VectorStore] Built context: {len(context_str)} chars from {len(results)} results")
        return context_str
