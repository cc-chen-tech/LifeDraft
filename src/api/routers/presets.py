...
async def create_preset(
    ...
    db = get_db()
    try:
        normalized_settings = normalize_preset_story_origin(
            strip_retired_wealth_keys(req.character_settings)
        )
        preset_id = db.save_character_preset(
            preset_name=req.preset_name,
            player_name=req.player_name,
            life_vision=req.life_vision,
            character_settings=normalized_settings,
            user_id=user_id,
        )
        return PresetInfo(
            preset_id=preset_id,
...
async def list_presets(
    ...
    return [
        PresetInfo(
            ...
            character_settings=normalize_preset_story_origin(
                strip_retired_wealth_keys(p.character_settings or {})  # type: ignore[arg-type]
            ),
...
async def get_preset(
    ...
    return PresetInfo(
        ...
        character_settings=normalize_preset_story_origin(
            strip_retired_wealth_keys(preset.get("character_settings", {}))
        ),
