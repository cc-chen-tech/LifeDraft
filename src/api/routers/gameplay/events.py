...
async def generate_event_sync(
    ...
    if game_loop.current_event and game_loop.current_event.options:
        logger.info(
            f"Returning existing event for game_id={game_id} (sync, options count: {len(game_loop.current_event.options)})"
        )
    ...
    if lock.locked():
        logger.warning(
            f"Event generation already in progress for game_id={game_id} (sync), returning current event"
        )
