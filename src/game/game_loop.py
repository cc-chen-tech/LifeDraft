...
class GameLoop(RoundSystemMixin):
    ...
    def start_new_game(self, initial_state: Optional[Dict[str, Any]] = None) -> PlayerState:
        ...
        if self.player_state.character_settings:
            ...
            logger.debug(f"Initialized {len(self.player_state.characters)} characters from settings")
    ...
    def load_game(self, state_dict: Dict[str, Any]) -> PlayerState:
        ...
        if not self.player_state.characters and self.player_state.character_settings:
            self.player_state.initialize_characters_from_settings()
            logger.debug(f"Initialized {len(self.player_state.characters)} characters from settings on load")
        ...
        if self.player_state.current_event_data:
            try:
                ...
                logger.info("Restored current event from saved state")
            except Exception as e:
                logger.warning(f"Failed to restore current event: {e}")
                self.current_event = None
        else:
            self.current_event = None
        ...
        week_decisions = [
            d for d in self.player_state.decision_history 
    ...
    def generate_weekly_event(self, stream_callback: Optional[Callable[[str], None]] = None, force: bool = False, status_callback: Optional[Callable[[str], None]] = None) -> Optional[GameEvent]:
        ...
        try:
            ...
            event = self.ai_generator.generate_event(
                ...
                activated_foreshadowing=NarrativeManager.select_foreshadowing_seed(self.player_state),
            ...
            if not event:
                ...
                logger.error(f"Player state: week={self.player_state.week}, age={self.player_state.age}")
    ...
    def advance_to_next_week(self) -> bool:
        ...
        # Save previous state for potential future use
        previous_state = self.player_state.to_dict()
        current_week = self.player_state.week
    ...
    def _generate_four_week_summary(self, current_week: int) -> None:
        ...
        try:
            ...
            summary_text = self.ai_generator.generate_four_week_summary(
                stories, decisions, character_settings, self.language,
                game_date_info=self.player_state.get_game_date_info()
            )
    ...
    def _generate_yearly_summary(self, current_week: int) -> None:
        ...
        relevant_summaries = [
            s for s in self.player_state.four_week_summaries
        ...
        try:
            ...
            summary_text = self.ai_generator.generate_yearly_summary(
                relevant_summaries, character_settings, start_week, current_week - 1, self.language,
                game_date_info=self.player_state.get_game_date_info()
            )
    ...
    def generate_summary(self, weeks: int = 10) -> Dict[str, Any]:
        ...
        if not period_decisions:
            return {
                ...
                "summary": "这段时间没有做出任何决策。" if self.language == "zh" else "No decisions made during this period.",
                "highlights": []
            }
    ...
    def _generate_fallback_event(self, is_round: bool = False) -> GameEvent:
        ...
        """
        logger.warning(f"Using {'round ' if is_round else ''}fallback event - AI generation failed!")
        
        character_settings = self.player_state.character_settings if self.player_state else {}
        
        if is_round:
            prefix = self.player_state.get_round_name(self.language) if self.player_state else "周一"
        else:
            prefix = ""
        ...
        if self.language == "zh":
            ...
            return GameEvent(
                ...
                options=[
                    EventOption(
                        text="Keep status quo and move forward" if not is_round else "Maintain the current rhythm",
                        effects={"energy": 0 if is_round else 5, "mood": 5, "knowledge": 0, "wealth": 0}
                    ),
                    EventOption(
                        text="Reflect on life direction" if not is_round else "Try something different",
                        effects={"energy": -5, "mood": 0, "knowledge": 5, "wealth": 0}
                    )
                ]
            )
