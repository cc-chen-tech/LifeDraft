...
def apply_character_effects(
    ...
    for name, effects in character_effects.items():
        ...
        if success:
            ...
            if character:
                event_types = ["deep_friendship", "conflict", "help_request", "secret_sharing", "betrayal_risk"]
...
def _generate_fallback_result(effects: Dict[str, Any], language: str) -> str:
    ...
    if effects.get("energy", 0) != 0:
        val = effects["energy"]
        changes.append(f"Energy {'+' if val > 0 else ''}{val}" if language == "en" 
                      else f"精力{'+' if val > 0 else ''}{val}")
    
    if effects.get("mood", 0) != 0:
        val = effects["mood"]
        changes.append(f"Mood {'+' if val > 0 else ''}{val}" if language == "en"
                      else f"情绪{'+' if val > 0 else ''}{val}")
    
    if effects.get("knowledge", 0) != 0:
        val = effects["knowledge"]
        changes.append(f"Knowledge {'+' if val > 0 else ''}{val}" if language == "en"
                      else f"学识{'+' if val > 0 else ''}{val}")
    
    if effects.get("wealth", 0) != 0:
        val = effects["wealth"]
        changes.append(f"Wealth {'+' if val > 0 else ''}¥{abs(val):,}" if language == "en"
