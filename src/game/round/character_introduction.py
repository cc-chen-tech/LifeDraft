...
class CharacterIntroductionService:
    ...
    def maybe_generate_new_character(self, probability: float = 0.08) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"新人物 {new_name} 已加入待引入队列（场景：{intro_context}，优先级：{pending_entry['priority']}）")
    ...
    def determine_introduction_context(self, new_person: Dict[str, Any]) -> str:
        ...
        # 根据角色类型判断
        if any(kw in role for kw in ["同事", "上司", "下属", "合作", "客户", "colleague", "boss", "partner", "client"]):
            return "work"
        elif any(kw in role for kw in ["同学", "老师", "导师", "student", "classmate", "teacher", "mentor"]):
            return "education"
    ...
    def check_introduction_opportunity(self) -> Optional[Dict[str, Any]]:
        ...
        sorted_pending = sorted(
            pending,
            key=lambda x: (-x.get("priority", 0), x.get("created_week", 0))
        )
        ...
        for entry in sorted_pending:
            ...
            if waiting_weeks >= 4 or attempts >= 3:
                logger.info(f"待引入人物 {entry['character_data'].get('name')} 已等待{waiting_weeks}周，强制引入")
                return entry
            
            # 检查是否有匹配的引入场景
            if self.matches_introduction_scene(intro_context):
                logger.info(f"发现匹配的引入场景 [{intro_context}]，准备引入 {entry['character_data'].get('name')}")
                return entry
    ...
    def matches_introduction_scene(self, intro_context: str) -> bool:
        ...
        if intro_context == "work":
            # 检查是否涉及工作相关内容
            work_keywords = ["公司", "工作", "项目", "会议", "同事", "办公室", "work", "office", "project", "meeting"]
            # 或者检查职业状态是否有变化
            career_records = world_model.get("career_records", {})
            player_name = player_state.player_name or "主角"
            if player_name in career_records:
            ...
            # 检查是否涉及社交活动
            social_keywords = ["聚会", "朋友", "餐厅", "酒吧", "party", "社交", "friend", "restaurant", "bar"]
    ...
    def introduce_pending_character(self, pending_entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
