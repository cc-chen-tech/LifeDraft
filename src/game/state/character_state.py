...
class CharacterState(BaseModel):
    ...
    mood_stability: int = Field(default=70, ge=0, le=100, description="情绪稳定性，影响情绪波动幅度")
    ...
    relationship_status: str = Field(
        default="single",
        description="感情状态: single/dating/engaged/married/divorced"
    )
    romantic_interest: str = Field(
        default="",
        description="暗恋/喜欢的对象名字（可以是主角或其他NPC）"
    )
    has_external_obstacle: bool = Field(
        default=False,
        description="是否有外部阻力（如家族反对），用于私奔事件"
    )
    peak_affinity: int = Field(
        default=50, ge=0, le=100,
        description="历史最高亲密度，用于判断反目成仇"
    )
    ...
    def update_relationship(self, affinity_change: int = 0, trust_change: int = 0, respect_change: int = 0) -> None:
        """
        更新与主角的关系属性。
