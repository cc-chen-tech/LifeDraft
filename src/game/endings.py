...
class EndingEvaluator:
    ...
    def evaluate_ending(
        ...
        relationship_score = sum(player_state.relationships.values()) / max(len(player_state.relationships), 1) / 100
    ...
    def _generate_summary(
        ...
        if self.ai_generator:
            try:
                ...
                if character_settings:
                    if language == "zh":
                        ...
                        if "era" in character_settings:
                            char_parts.append(f"时代：{character_settings['era'].get('year', '')}年")
                        if "age" in character_settings:
                            char_parts.append(f"起始年龄：{character_settings['age'].get('age', '')}岁")
                        if "gender" in character_settings:
                            char_parts.append(f"性别：{character_settings['gender'].get('gender', '')}")
                        ...
                        if "age" in character_settings:
                            char_parts.append(f"Starting age: {character_settings['age'].get('age', '')}")
                        if "gender" in character_settings:
                            char_parts.append(f"Gender: {character_settings['gender'].get('gender', '')}")
                        if "traits" in character_settings:
                            traits = character_settings["traits"]
                ...
                summary_texts = [s.get("summary", "") for s in four_week_summaries[-10:]] if four_week_summaries else []
                
                decision_history = [d.get("choice", "") for d in player_state.decision_history[-10:]]
...
Be vivid and specific, reflecting the character's uniqueness."""
    ...
    def _calculate_achievements(
        ...
        perfect_weeks = sum(1 for _ in range(len(player_state.decision_history))
                           if player_state.energy > 70 and player_state.mood > 70 and player_state.knowledge > 70)
        if perfect_weeks > 0:
            achievements.append(f"{perfect_weeks}个完美周" if language == "zh" else f"{perfect_weeks} Perfect Weeks")
