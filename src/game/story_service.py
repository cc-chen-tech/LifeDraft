...
class StoryService:
    ...
    def _validate_and_retry_continuation(
        ...
        try:
            ...
            validation = validator.validate_story(
                ...
                player_state_dict=player_state if isinstance(player_state, dict) else player_state.to_dict(),
                character_settings=character_settings,
                language=self.language,
            )
            ...
            if not validation.has_critical_issues:
                logger.info(f"[StoryContinuation] 一致性校验有 {len(validation.warning_issues)} 个WARNING，不触发重试")
                return continuation
            ...
            if retry_continuation:
                logger.info(f"[StoryContinuation] 重试生成完成，故事长度: {len(retry_continuation)}")
    ...
    def compress_story(self, story: str, choice: str, pending_storylines: Optional[List] = None, established_facts: Optional[List] = None, character_habits: Optional[List] = None) -> Dict[str, Any]:
        """
        ...
        """
        return self.ai_generator.compress_story(story, choice, self.language, pending_storylines, established_facts, character_habits)

    def compress_narrative(self, story: str, choice: str, pending_storylines: Optional[List] = None) -> Dict[str, Any]:
        """Narrative compression only (parallel-friendly). Returns summary + event_concluded + storyline_updates."""
        return self.ai_generator.compress_narrative(story, choice, self.language, pending_storylines)

    def extract_world_updates(self, story: str, choice: str, established_facts: Optional[List] = None, character_habits: Optional[List] = None) -> Dict[str, Any]:
        """World state extraction only (parallel-friendly). Returns fact/habit/location/career/commitment/causal/foreshadowing updates."""
        return self.ai_generator.extract_world_updates(story, choice, self.language, established_facts, character_habits)
