...
class WorldModel:
    ...
    def __init__(self):
        ...
        # ★ 新增字段：位置图谱与角色知识集
        self.location_graph: Dict[str, Dict[str, int]] = (
            {}
        )  # {"城市A": {"城市B": 2, "城市C": 3}}
        self.character_knowledge_sets: Dict[str, set] = (
            {}
        )  # {"角色A": {"秘密1", "事件2"}}
    ...
    def from_player_state(cls, player_state) -> "WorldModel":
        ...
        if "occupation" in cs and protagonist_name not in wm.career_records:
            occ = cs["occupation"]
            career = CareerInfo(
    ...
    def build_constraints_text(self, language: str) -> str:
        ...
        required_cast_lines = self._build_required_cast_constraints(
            "zh" if zh else "en"
        )
        ...
        if self.continuity_ledger is not None:
            ledger_lines = self.continuity_ledger.build_constraints_text(
                "zh" if zh else "en"
            )
            if ledger_lines:
