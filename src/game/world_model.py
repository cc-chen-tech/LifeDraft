...
class LocationInfo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"location": self.location, "region": self.region,
                "since_week": self.since_week, "travel_mode": self.travel_mode}

    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "LocationInfo":
        return cls(location=d.get("location", ""), region=d.get("region", ""),
                   since_week=d.get("since_week", 0), travel_mode=d.get("travel_mode", "resident"))
...
class CareerInfo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"current_job": self.current_job, "employer": self.employer,
                "level": self.level, "since_week": self.since_week, "history": self.history}

    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "CareerInfo":
        return cls(current_job=d.get("current_job", ""), employer=d.get("employer", ""),
                   level=d.get("level", "mid"), since_week=d.get("since_week", 0),
...
class Commitment:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"description": self.description, "parties": self.parties,
                "deadline_week": self.deadline_week, "status": self.status,
                "created_week": self.created_week, "importance": self.importance,
                "scheduled_round": self.scheduled_round, "scheduled_week": self.scheduled_week,
                "event_hint": self.event_hint}

    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "Commitment":
        return cls(description=d.get("description", ""), parties=d.get("parties", []),
                   deadline_week=d.get("deadline_week", -1), status=d.get("status", "pending"),
                   created_week=d.get("created_week", 0), importance=d.get("importance", "normal"),
                   scheduled_round=d.get("scheduled_round", -1), scheduled_week=d.get("scheduled_week", -1),
...
class CausalChain:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"cause": self.cause, "expected_consequence": self.expected_consequence,
                "characters": self.characters, "created_week": self.created_week,
                "resolved": self.resolved}

    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "CausalChain":
        return cls(cause=d.get("cause", ""), expected_consequence=d.get("expected_consequence", ""),
                   characters=d.get("characters", []), created_week=d.get("created_week", 0),
...
class PhysicalState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"condition": self.condition, "severity": self.severity,
                "since_week": self.since_week, "expected_recovery_week": self.expected_recovery_week}

    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> "PhysicalState":
        return cls(condition=d.get("condition", ""), severity=d.get("severity", "moderate"),
...
class CharacterProfile:
    ...
    behavioral_boundaries: List[str] = field(default_factory=list) # ["绝不在公开场合发怒", "不会背叛朋友"]
...
class WorldModel:
    ...
    def __init__(self):
        ...
        self.character_profiles: Dict[str, CharacterProfile] = {}  # Behavioral profiles per character
        ...
        for fact in getattr(player_state, "established_facts", []):
            ...
            if category == "location" and subject not in wm.character_locations:
                wm.character_locations[subject] = LocationInfo(
                    location=fact_text, region=_extract_region(fact_text),
                    since_week=week, travel_mode="resident"
                )
    ...
    def check_geographic_feasibility(self, char_names: List[str]) -> List[str]:
        ...
        located = {n: self.character_locations[n] for n in char_names if n in self.character_locations}
        if len(located) < 2:
    ...
    def check_career_plausibility(self, character: str, new_role: str, new_level: str = "") -> List[str]:
    ...
    def get_established_profile_names(self) -> List[str]:
        ...
        return [name for name, p in self.character_profiles.items()
    ...
    def build_constraints_text(self, language: str) -> str:
        ...
        header = "\n【世界模型约束 — 必须严格遵守，不得矛盾】" if zh else \
                 "\n[World Model Constraints — MUST STRICTLY FOLLOW, NO CONTRADICTIONS]"
        footer = ("❗以上所有约束在故事生成时必须严格遵守。如需变动（如人物搬迁、职位变化），"
                  "必须在故事中给出合理的交代和过渡。") if zh else \
    ...
    def _build_commitment_constraints(self, zh: bool) -> str:
        ...
        non_urgent = [c for c in pending if c.deadline_week < 0 or get_effective_deadline(c) > self.current_week + 4]
        ...
        if zh:
            ...
            if urgent:
                ...
                for c in urgent:
                    ...
                    if c.importance == "critical" and c.deadline_week <= self.current_week:
                        ...
                        overdue = "（已过期！）" if c.deadline_week <= self.current_week else f"（第{c.deadline_week}周截止）"
                    lines.append(f"  - [{c.importance}] {c.description} — 涉及：{'、'.join(c.parties)}{overdue}")
            if non_urgent:
            ...
            if urgent:
                lines.append("  Warning: The following are due soon or overdue — must be reflected:")
                for c in urgent:
                    # ★ 关键承诺显示为"待兑现"而非"已过期"
                    if c.importance == "critical" and c.deadline_week <= self.current_week:
                        overdue = "(CRITICAL - pending fulfillment)"
                    else:
                        overdue = "(OVERDUE!)" if c.deadline_week <= self.current_week else f"(due week {c.deadline_week})"
                    lines.append(f"  - [{c.importance}] {c.description} — parties: {', '.join(c.parties)} {overdue}")
            if non_urgent:
    ...
    def _build_causal_constraints(self, zh: bool) -> str:
        ...
        if zh:
            ...
            lines.append("  Warning: These cause-effect chains should manifest in the story at appropriate times.")
        return "\n".join(lines)
    ...
    def _build_dynamic_facts_constraints(self, zh: bool) -> str:
        ...
        if zh:
            ...
            for f in display_facts:
                ...
                prefix = f"[{importance_label}] " if importance_label else ""
                lines.append(f"  - {prefix}{f.constraint_text}")
            lines.append("  Warning: Above constraints were auto-extracted from story history. Strictly follow them.")
        return "\n".join(lines)
    ...
    def _build_character_profile_constraints(self, zh: bool) -> str:
        ...
        """
        established = {name: p for name, p in self.character_profiles.items()
        ...
        if zh:
            ...
            lines.append("  Warning: Profiles were synthesized from multiple story rounds. Character speech,")
            lines.append("  behavior, decisions, and emotions MUST be consistent. Any growth/change needs explicit story justification.")
        return "\n".join(lines)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "character_locations": {n: loc.to_dict() for n, loc in self.character_locations.items()},
...
def _extract_region(location_text: str) -> str:
    ...
    common_prefixes = [
        ...
        "武汉", "南京", "西安", "长沙", "苏州", "郑州", "东莞", "青岛",
        "沈阳", "大连", "厦门", "昆明", "贵阳", "福州", "济南", "哈尔滨",
        "长春", "合肥", "南宁", "太原", "石家庄", "兰州", "乌鲁木齐",
        "呼和浩特", "拉萨", "银川", "西宁", "海口", "香港", "澳门", "台北",
    ]
