...
- `/api/images/*`：角色/物品/场景图生成、历史图、SSE 场景图事件（需认证）
- `/api/collection/*`：角色/物品/地标收集与补图（支持批量生成）
- `/api/voice-reading/*`：MiniMax 章节朗读任务、分段音频、设置与续播进度
- `/api/presets/*`：角色预设保存/加载

成就与结局：

- `/api/games/{id}/ending`：结局生成（含成就计算与人生回顾）
- `/api/achievements/*`：成就查询与徽章状态
...
- **SSE 场景图事件（`/api/games/{id}/scene-events`）已要求认证**，未登录返回 401。
