...
- **开发与测试页 (04)**：补充 security 契约测试系列（C-01~C-07：JWT secret、硬编码密钥、SSE auth、图片 base64、SQLAlchemy raw SQL、pickle 禁用、prompt injection）说明。
- **状态与数据页 (07)**：补充 `GamePlaylist`、`achievements`、`life_reviews`、4D resources、决策历史扩展。
- **排障页 (08)**：新增”SSE 场景图事件 401”、”JWT 签名失败”、”SSE 502/504 网关错误”排障条目；更新音乐播放排障（混合缓存池）。
...
- **发布清单页 (10)**：补充 security 检查项（硬编码密钥、SSE auth、pickle/raw SQL、prompt injection 防护）。
