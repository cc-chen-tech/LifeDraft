...
问题：网易云基础歌匹配仍差，出现 `都选C`、`可爱女人` 等明显不适合现代职场场景的歌曲。AI 生成耗时期间 UI 反馈不够清楚的问题已在本轮修复。
...
- 2026-06-09 继续补洞：scheduled event 解析到 JSON 后旧逻辑会直接返回，不经过 quick validator。现在 scheduled event 返回前也会校验预设人物使用情况；若第一次输出把导师/闺蜜/同期替换成“马老板/方蕾”等名单外人物，会带着失败原因重试一次，并只返回校验通过的故事。
- 回归测试：
  - `test_scheduled_event_prompt_inherits_story_authority_constraints`
  - `test_scheduled_event_generation_retries_when_story_replaces_preset_cast`
...
- 红灯复现：
  ...
  - 旧 scheduled event 生成拿到第一版 JSON 后直接返回，`test_scheduled_event_generation_retries_when_story_replaces_preset_cast` 中 `client.calls == 1`，不会触发重试。
...
- 本地验证：
  ...
  - `pytest tests/test_preset_cast_authority_contract.py::test_scheduled_event_generation_retries_when_story_replaces_preset_cast -q`
...
- 继续修复一个播放反馈遗漏：当网易云基础曲已经可播放、MiniMax 原创音乐仍在后台生成时，播放器会显示“正在生成原创场景音乐，完成后加入下一首”，避免用户误以为原创音乐没有触发。
...
- 回归测试补充：`有基础歌曲时仍提示 MiniMax 原创音乐正在后台生成` 先复现旧 UI 没有生成提示，再验证基础歌曲保留且生成状态可见。
- 生产复测：
