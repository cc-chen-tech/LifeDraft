...
- [x] 1.4 Add a frontend regression test proving `/saves` does not render stale `savedGames` when the current user is unauthenticated.
...
- [x] 2.3 Gate `/saves` rendering through current authentication state so stale in-memory save lists cannot leak between sessions.
