...
#### Scenario: Real-provider music generation is enqueued without blocking the API request
- **GIVEN** MiniMax music generation is enabled with real provider credentials
- **AND** deterministic local-audio mode is not enabled
...
#### Scenario: Local deterministic generated music remains synchronously verifiable
- **GIVEN** MiniMax local-audio mode is enabled for deterministic tests
- **WHEN** the client requests `/api/music/generate`
- **THEN** the API MAY return HTTP 200 with a ready generated track
