...
def test_music_playlist_routes_reject_unauthenticated_and_other_users_real_db(
    client: TestClient,
) -> None:
    init_db()
    os.environ.setdefault("JWT_SECRET", "music-playlist-owner-test-secret")
    suffix = uuid4().hex[:12]
    ...
    try:
        owner = User(
            private_id=f"playlist-owner-{suffix}",
            public_id=f"PO{suffix[:6]}",
            display_name=f"Playlist Owner {suffix}",
        )
        intruder = User(
            private_id=f"playlist-intruder-{suffix}",
            public_id=f"PI{suffix[:6]}",
            display_name=f"Playlist Intruder {suffix}",
        )
        session.add_all([owner, intruder])
        session.flush()
        created_user_ids.extend([int(owner.user_id), int(intruder.user_id)])

        game = Game(
            user_id=owner.user_id,
            language="zh",
            initial_state={"player_name": f"Playlist Owner Save {suffix}"},
        )
        session.add(game)
        session.flush()
        created_game_ids.append(int(game.game_id))
        session.add(
            GamePlaylist(
                game_id=game.game_id,
                current_song_json={"id": 501, "name": f"Owner Current {suffix}"},
                queue_json=[{"id": 502, "name": f"Owner Next {suffix}"}],
                is_playing=True,
        ...
        unauthenticated = client.get(f"/api/music/playlist/{game_id}")
        assert unauthenticated.status_code == 401
        assert f"Owner Current {suffix}" not in unauthenticated.text

        token = create_token(int(intruder.user_id))
        headers = {"Authorization": f"Bearer {token}"}

        read_response = client.get(f"/api/music/playlist/{game_id}", headers=headers)
        update_response = client.put(
            ...
            json={
                "songs": [
                    {
                        "id": 601,
                        "name": f"Intruder Song {suffix}",
                        "artists": ["X"],
                        "album": "Y",
                        "duration": 1000,
                        "url": "https://example.com/intruder.mp3",
                        "source": "netease",
                    }
                ]
            },
        ...
        sync_response = client.post(
            ...
            headers=headers,
            json={"current_position_ms": 99999, "is_playing": False, "volume": 0.1},
        )
        advance_response = client.post(
            f"/api/music/playlist/{game_id}/advance",
            headers=headers,
        )
        ...
        session.expire_all()
        playlist = session.query(GamePlaylist).filter_by(game_id=game_id).one()
        assert playlist.current_song_json["name"] == f"Owner Current {suffix}"
        assert playlist.queue_json[0]["name"] == f"Owner Next {suffix}"
        ...
        session.rollback()
        if created_game_ids:
            session.query(GamePlaylist).filter(GamePlaylist.game_id.in_(created_game_ids)).delete(
                synchronize_session=False
            )
            session.query(GameState).filter(GameState.game_id.in_(created_game_ids)).delete(
                synchronize_session=False
...
def test_music_generation_routes_reject_other_users_game_real_db(
    client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    init_db()
    os.environ.setdefault("JWT_SECRET", "music-generation-owner-test-secret")
    monkeypatch.setenv("MINIMAX_E2E_LOCAL_AUDIO", "1")
    monkeypatch.delenv("MINIMAX_API_KEY", raising=False)

    from src.api.routers import music as music_router

    background_calls: list[tuple[int, str, dict]] = []
    monkeypatch.setattr(
        music_router,
        "_generate_music_in_background",
        lambda game_id, story_text, analysis: background_calls.append(
            (game_id, story_text, analysis)
        ),
    )
    ...
    try:
        owner = User(
            private_id=f"music-gen-owner-{suffix}",
            public_id=f"GO{suffix[:6]}",
            display_name=f"Music Gen Owner {suffix}",
        )
        intruder = User(
            private_id=f"music-gen-intruder-{suffix}",
            public_id=f"GI{suffix[:6]}",
            display_name=f"Music Gen Intruder {suffix}",
        )
        session.add_all([owner, intruder])
        session.flush()
        created_user_ids.extend([int(owner.user_id), int(intruder.user_id)])
        ...
        game = Game(
            user_id=owner.user_id,
            language="zh",
            initial_state={"player_name": f"Music Gen Owner Save {suffix}"},
        )
        session.add(game)
        session.flush()
        created_game_ids.append(int(game.game_id))
        ...
        intruder_headers = {"Authorization": f"Bearer {create_token(int(intruder.user_id))}"}
        payload = {
            "game_id": game_id,
            "story_text": f"只属于 owner 的故事 {suffix}",
            "analysis": {"mood": "紧张", "scene_type": "追逐", "environment": "现代都市"},
        }
