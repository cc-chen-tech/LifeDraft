...
def _event_type_and_payload(frame: str) -> tuple[str, int | None, object]:
...
async def test_choice_stream_emits_status_story_and_complete_with_real_cache() -> None:
    game_id = 783_001
    loop = _ChoiceLoop()
    session = GameLoopSession(game_loop=loop, game_id=game_id)

    frames = [frame async for frame in stream_choice(loop, 2, game_id, session=session)]
...
async def test_choice_stream_replays_cached_story_before_new_output() -> None:
    ...
    frames = [
        frame
        async for frame in stream_choice(loop, 1, game_id, session=session, last_event_id=-1)
    ]
    events = [_event_type_and_payload(frame) for frame in frames]
...
async def test_choice_stream_data_error_emits_error_and_clears_cache() -> None:
    game_id = 783_003
    loop = _ChoiceLoop(fail=True)
    session = GameLoopSession(game_loop=loop, game_id=game_id)
    session.cache_sse_chunk("stale-story")

    frames = [frame async for frame in stream_choice(loop, 0, game_id, session=session)]
    events = [_event_type_and_payload(frame) for frame in frames]
