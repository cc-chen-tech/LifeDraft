...
class TestGenerationStateMetricsPersistence:
    ...
    def test_harness_metrics_stores_generation_state(self, tmp_path):
        ...
        metrics_output = {
            ...
            "transitions": [
                ...
                {"reason": "harness_retry", "temperature": 0.9, "model": "deepseek-chat"},
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT game_id, week, attempts, final_score, latency_ms FROM generation_runs WHERE id = ?", (run_id,))
        row = cursor.fetchone()
        conn.close()
    ...
    def test_generation_state_metrics_with_all_transition_reasons(self, tmp_path):
        ...
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT evidence, details FROM constraint_checks WHERE run_id = ?", (run_id,))
        row = cursor.fetchone()
        conn.close()
...
class TestParallelPostProcessingDataIntegrity:
    ...
    def test_parallel_postprocessing_save_read_roundtrip(self, db_session, game_ctx):
        ...
        assert loaded_pp["compression_result"]["key_events_preserved"] == ["遇见李白", "饮酒作诗"]
    ...
    def test_concurrent_world_model_writes_no_corruption(self, db_session, game_ctx):
        ...
        base_state = {
            "player_name": "并发测试",
            "energy": 100, "mood": 100, "knowledge": 50, "wealth": 500,
            "week": 3, "age": 25,
            "world_model_data": {
                "character_locations": {},
                "career_records": {},
        ...
        t1 = threading.Thread(target=apply_update, args=("character_locations", updates_a["character_locations"]))
        t2 = threading.Thread(target=apply_update, args=("career_records", updates_b["career_records"]))
        t1.start()
        t2.start()
        t1.join()
        t2.join()
    ...
    def test_compression_result_in_state_json(self, db_session, game_ctx):
        ...
        state_json = {
            "player_name": "压缩测试",
            "energy": 90, "mood": 75, "knowledge": 60, "wealth": 700,
            "week": 3, "age": 25,
...
class TestBackwardCompatibility:
    ...
    def test_old_world_model_data_format_still_works(self, db_session, game_ctx):
        ...
        state_json = {
            "player_name": "旧WM玩家",
            "energy": 90, "mood": 80, "knowledge": 55, "wealth": 600,
            "week": 4, "age": 26,
...
class TestFeatureFlagNonPersistence:
    ...
    def test_feature_flag_state_not_persisted_in_db(self, db_session, game_ctx):
        ...
        state_json = {
            "player_name": "Flag测试",
            "energy": 100, "mood": 100, "knowledge": 50, "wealth": 1000,
            "week": 1, "age": 25,
        }
    ...
    def test_feature_flag_changes_dont_affect_saved_games(self, db_session, game_ctx):
        ...
        state_json = {
            ...
            "energy": 80, "mood": 75, "knowledge": 60, "wealth": 900,
            "week": 3, "age": 26,
            "world_model_data": {
        ...
        assert loaded.state_json["world_model_data"]["character_locations"]["小红"]["location"] == "上海"
...
class TestModelFallbackTracking:
    ...
    def test_model_used_field_persisted_in_game_event(self, db_session, game_ctx):
        ...
        state_json = {
            "player_name": "模型降级测试",
            "energy": 90, "mood": 80, "knowledge": 55, "wealth": 1200,
            "week": 7, "age": 27,
            "current_event_data": {
    ...
    def test_model_fallback_chain_persisted(self, db_session, game_ctx):
        ...
        state_json = {
            "player_name": "降级链测试",
            "energy": 85, "mood": 70, "knowledge": 50, "wealth": 1000,
            "week": 10, "age": 28,
            "generation_metadata": {
                "model_used": "qwen-max",
    ...
    def test_multiple_game_states_with_different_models(self, db_session, game_ctx):
        ...
        for i, model in enumerate(models):
            state_json = {
                "player_name": "多模型测试",
                "energy": 100 - i * 5,
                "mood": 80, "knowledge": 50, "wealth": 1000,
                "week": i + 1, "age": 25,
                "current_event_data": {
            ...
            gs = _save_game_state(db_session, game.game_id, week=i + 1, age=25, state_json=state_json)
