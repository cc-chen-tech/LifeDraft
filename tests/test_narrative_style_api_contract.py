...
def test_games_router_has_narrative_style_list_endpoint():
    ...
    with open("src/api/routers/games.py", "r") as f:
        content = f.read()
    assert "narrative-style-options" in content or "narrative_style_options" in content, \
...
def test_games_router_has_narrative_style_get_endpoint():
    ...
    assert "narrative-style" in content, \
        "games.py 缺少 GET narrative-style 端点"
...
def test_games_router_has_narrative_style_update_endpoint():
    ...
    with open("src/api/routers/games.py", "r") as f:
        content = f.read()
    # 应有 PUT 方法
    assert any(kw in content for kw in ['put("', "put('/", '@router.put']), \
...
def test_frontend_settings_has_story_style_option():
    ...
    with open("frontend/src/app/play/page.tsx", "r") as f:
        content = f.read()
    assert any(kw in content for kw in ["故事风格", "storyStyle", "narrativeStyle", "narrative-style"]), \
        "前端设置菜单缺少故事风格选项"
