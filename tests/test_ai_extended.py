...
class TestOptionGenerator:
    ...
    def test_generate_options_success(self):
        ...
        mock_client.call.return_value = json.dumps({
            "options": [
                {"text": "选项A", "effects": {"energy": -5, "mood": 10, "knowledge": 0, "wealth": 0}},
                {"text": "选项B", "effects": {"energy": 0, "mood": 0, "knowledge": 10, "wealth": 0}},
            ]
        })
    ...
    def test_generate_options_fallback_en(self):
        ...
        assert "positively" in event.options[0].text.lower() or "face" in event.options[0].text.lower()
    ...
    def test_validate_options_consistency_character_in_story_text(self):
        ...
        issues = gen.validate_options_consistency(event, story_text, available_people, language="zh")
...
class TestDynamicFact:
    ...
    def test_to_dict_and_from_dict(self):
        ...
        fact = DynamicFact(
            fact_id="f_test_1", fact_type="physical_state",
            subject="张三", description="受伤了",
            constraint_text="不能跑步", source_week=5,
            importance="critical"
        )
        d = fact.to_dict()
...
class TestStoryAnalyzer:
    ...
    def test_analyze_story_success(self):
        ...
        results = analyzer.analyze_story(
            "张三在事故中受伤了", "去医院", [], 5,
            {"relationships": {"key_people": [{"name": "张三"}]}}, "zh"
        )
    ...
    def test_parse_update_action(self):
        ...
        analyzer = StoryAnalyzer(Mock())
        existing = [DynamicFact(
            fact_id="f_old", fact_type="physical_state",
            subject="张三", description="受伤",
            constraint_text="不能跑步", active=True
        )]
        response = json.dumps({
            "facts": [{
                "action": "update",
    ...
    def test_parse_invalidate_action(self):
        ...
        analyzer = StoryAnalyzer(Mock())
        existing = [DynamicFact(
            fact_id="f_old", fact_type="test",
            subject="A", description="d",
            constraint_text="c", active=True
        )]
        response = json.dumps({
            "facts": [{"action": "invalidate", "target_fact_id": "f_old",
                       "subject": "A", "description": "d"}]
        })
    ...
    def test_build_existing_facts_context_zh(self):
        ...
        analyzer = StoryAnalyzer(Mock())
        facts = [DynamicFact(
            fact_type="test", subject="张三",
            description="有伤", constraint_text="别跑", active=True
        )]
    ...
    def test_build_existing_facts_context_en(self):
        ...
        analyzer = StoryAnalyzer(Mock())
        facts = [DynamicFact(
            fact_type="test", subject="John",
            description="injured", constraint_text="no running", active=True
        )]
    ...
    def test_parse_duplicate_id(self):
        ...
        analyzer = StoryAnalyzer(Mock())
        existing = [DynamicFact(
            fact_id="f_张三_physical__w5", fact_type="physical_state",
            subject="张三", description="old"
        )]
        response = json.dumps({
            "facts": [{
                "action": "new", "fact_type": "physical_state",
                "subject": "张三", "description": "new",
...
class TestStoryGenerator:
    ...
    def test_generate_round_event_fallback(self, mock_sys, mock_prompt):
        ...
        event = gen.generate_round_event(
            ...
            language="zh", round_number=0, round_context="",
    ...
    def test_generate_round_event_en_fallback(self, mock_sys, mock_prompt):
        ...
        gen = StoryGenerator(mock_client)
        event = gen.generate_round_event(
            player_state={"week": 0}, language="en",
            round_number=0, round_context="", option_generator=Mock()
        )
...
class TestStoryRewriter:
    ...
    def test_rewrite_segment_success(self):
        ...
        result = rewriter.rewrite_story_segment(
            "Original story", "segment to replace",
            "Make it funnier", None, "", language="zh"
        )
...
class TestSummaryGenerator:
    ...
    def test_clean_summary_text_json_prefix(self):
        from src.ai.summary_generator import SummaryGenerator
        assert "summary" not in SummaryGenerator._clean_summary_text('{"summary": "Clean text"}').lower().split()[:1]
