...
def test_sse_replay_keeps_tail_event_ids_after_cache_trimming() -> None:
    ...
    for index in range(session.MAX_SSE_CACHE_SIZE + 2):
        assert session.cache_sse_chunk(f"chunk-{index}") == index
...
def test_options_cache_requires_matching_story_and_prefetch_lifecycle() -> None:
    session = GameLoopSession(object(), game_id=8)
    options = [{"id": "investigate", "text": "Inspect the letter"}]
    session.set_cached_options(week=4, round_num=2, options=options, story_content="first story")
...
def test_store_isolates_owner_keys_preserves_cache_on_update_and_cleans_expired_sessions() -> None:
    store = SessionStore()
    first_loop = object()
    first = store.put(15, first_loop, user_id=1)
