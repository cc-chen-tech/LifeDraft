                ...
                if isinstance(node, ast.JoinedStr):
                    for value in node.values:
                        if isinstance(value, ast.Constant) and isinstance(value.value, str):
                            ...
                            if any(kw in lowered for kw in ["select ", "insert ", "update ", "delete ", "drop ", "create ", "alter "]):
                                # 排除日志语句：检查整行是否包含 logger.
                                line = lines[node.lineno - 1] if node.lineno <= len(lines) else ""
                                if "logger." in line or "log." in line:
                                    continue
                                violations.append(f"{py_file.relative_to(db_dir.parent.parent)}:{node.lineno}: f-string SQL")
