...
export default function EndingPage() {
  ...
  const achievements = (endingData?.achievements as { list?: string[] })?.list || [];
  
  // ★ 提取 final_stats
  const finalStats = endingData?.final_stats as Record<string, unknown> | undefined;
  ...
  const relationships = finalStats?.relationships as Record<string, number> | undefined;
      ...
      <div className="w-full max-w-[65ch] space-y-8 text-center">
        ...
        {isLoading ? (
          ...
          <>
            ...
            {finalStats && (
              <div className="py-4">
                <h3 className="text-sm font-medium text-foreground mb-4">最终状态</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <p className="text-xs text-muted-foreground mb-1">精力</p>
                    <span className="text-2xl font-bold text-emerald-400">{energy ?? "--"}</span>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-muted-foreground mb-1">情绪</p>
                    <span className="text-2xl font-bold text-sky-400">{mood ?? "--"}</span>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-muted-foreground mb-1">学识</p>
                    <span className="text-2xl font-bold text-violet-400">{knowledge ?? "--"}</span>
                  </div>
                  <div className="text-center">
                    <p className="text-xs text-muted-foreground mb-1">财富</p>
                    <span className="text-2xl font-bold text-amber-400">¥{wealth?.toLocaleString() ?? "--"}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {relationships && Object.keys(relationships).length > 0 && (
              <Card className="p-4 bg-card border-border text-left">
                <h3 className="text-sm font-medium text-foreground mb-3">人际关系</h3>
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(relationships).map(([name, affinity]) => (
                    <div key={name} className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">{name}</span>
                      <span className="text-primary font-medium">{affinity}/100</span>
                    </div>
                  ))}
                </div>
              </Card>
